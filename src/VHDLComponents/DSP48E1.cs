...
namespace SME.VHDLComponents
{
	public abstract class DSP48E1<TA, TB, TC, TD, TOut> : SimpleProcess, IVHDLComponent
	{
		...
		private string FormatOutString(string componentname, int indentation, string str)
		{
			...
			return ind + string.Join(Environment.NewLine + ind, string.Format(str.Trim(), Renderer.ConvertToValidVHDLName(this.GetType().Name), "DSP48E1", InputAWidth - 1, InputBWidth - 1, InputCWidth - 1, InputDWidth - 1, OutputWidth - 1, zeroes_for_a, zeroes_for_b, zeroes_for_c, zeroes_for_d).Replace("\t", new string(' ', 4)).Replace("\r", "").Split(new string[] { "\n" }, StringSplitOptions.None));
		}

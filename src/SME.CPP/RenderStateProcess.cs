...
namespace SME.CPP
{
	public class RenderStateProcess
	{
        ...
        public IEnumerable<string> DeclareVariables(IEnumerable<Variable> variables, int indentation)
        {
            if (variables != null && variables.Any())
			{
				...
				foreach (var n in variables)
                    yield return $"{indent}{Parent.TypeScope.GetType(n).Name} {n.Name};";
		...
		{
			return string.Format("({0}){1}", Parent.TypeScope.GetType(e.SourceResultType).Name, RenderExpression(e.Expression));
		}
